...
our ( $from, $author, %metadata, @commands, $tmpdir, $tmpcount, $prefix, $num_args, $docker_path, $repository, $tag );

if (@ARGV < 3) {
  print "\nUsage: docker_compile.pl path_to_dockerfile repository tagname\n";
  exit;
}
$docker_path=$ARGV[0];
$repository=$ARGV[1];
...
chdir($docker_path) or die "Cant chdir to $docker_path $!";
open DOCKERFILE, "<Dockerfile" or die "Can't open Dockerfile: $!\n";;
while ( <DOCKERFILE> ) {
  chomp;
...
# NOTE: repository and tag have been passed on command line
